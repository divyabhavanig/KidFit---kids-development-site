document.addEventListener('DOMContentLoaded', function () {
    ...
    const checkbox = document.getElementById('planting-checkbox');
    const congratsContainer = document.getElementById('congrats-container');
    const starRating = 5;

    // Create the container variable outside the event listener
    ...
    submitButton.addEventListener('click', function (event) {
        ...
        if (checkbox.checked) {
            ...
            alert('Please check the "Did you Plant?" checkbox before submitting.');
        }
    });
